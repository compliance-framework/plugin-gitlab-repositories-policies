...
check-opa: # Verify the OPA CLI is installed
	@command -v opa >/dev/null 2>&1 || { echo "opa CLI not found. Please install it: https://www.openpolicyagent.org/docs/latest/cli/"; exit 1; }

##@ Policies
test: check-opa ## Test policy files
	@opa test policies

validate: check-opa ## Validate policy files
	@opa check policies
...
build: clean check-opa ## Build the policy bundle
	@mkdir -p dist/

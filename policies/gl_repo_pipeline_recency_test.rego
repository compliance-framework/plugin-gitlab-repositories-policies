...
test_pipeline_just_within_limit_ok if {
    # ran just inside max_age_days to avoid flakiness from separate time.now_ns() calls
    max_ns := policy.max_age_days * 24 * 60 * 60 * 1000 * 1000 * 1000
    within_limit_ts := time.format(time.now_ns() - (max_ns - (1 * 1000 * 1000 * 1000)))
    inp := {"last_pipeline_run": {"updated_at": within_limit_ts, "status": "success"}}
    v := count(policy.violation) with input as inp
    v == 0
}
